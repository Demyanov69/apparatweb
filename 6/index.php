...
try {
    if ($is_auth) {
        ...
        $stmt = $db->prepare("DELETE FROM application_languages WHERE application_id IN (SELECT id FROM applications WHERE user_id = ?)");
        $stmt->execute([$user_id]);

        $stmt = $db->prepare("INSERT INTO application_languages (application_id, language_id) SELECT id, ? FROM applications WHERE user_id = ?");
        foreach ($languages as $lang_id) {
            $stmt->execute([$lang_id, $user_id]);
        }
